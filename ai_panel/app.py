...
BASE_SYSTEM = (
    ...
    "【書式ルール（厳守）】"
    "① 回答は必ずMarkdown記法（## 見出し・箇条書き -・**強調**など）のみで記述すること。"
    "② HTMLタグ（<h1> <ul> <li> <p> <br> など）は絶対に使用禁止。Markdown記法のみを使うこと。"
    "③ プロンプトを生成・提案する場合は、プロンプト本体を <prompt></prompt> タグで囲み、"
    "タグ内もMarkdown形式で整形すること。"
)
...
def step1():
    ...
    prompt = (
        f"【回答ルール（必ず守ること）】\n"
        f"・HTMLタグ（<h1>/<ul>/<li>等）は使用禁止。##・-・**などMarkdown記法のみを使うこと。\n"
        f"・回答全体を必ず6,000文字以内に収めること。超える場合は要点を絞り要約して収めること。\n"
        f"・プロンプト生成時は本文を <prompt></prompt> タグで囲むこと。\n\n"
        f"以下の質問に詳しく回答してください。\n"
        f"・主要なポイントを明確に述べること\n"
        f"・具体例や根拠を含めること\n"
